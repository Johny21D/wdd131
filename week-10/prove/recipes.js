...
const recipeTemplate = (recipe) => `
    <section class="recipe-card">
        ...
        <div class="recipe-content">
            <div class="tags-container">${recipe.tags.map(tag => `<span class="tag">${tag}</span>`).join('')}</div>
            <h2>${recipe.name}</h2>
            <span class="rating" role="img" aria-label="Rating: ${recipe.rating} stars">${renderStars(recipe.rating)}</span>
...
const renderStars = (rating) => {
    let stars = '';
    for (let i = 1; i <= 5; i++) {
        stars += `<span class="${i <= Math.floor(rating) ? 'icon-star' : 'icon-star-empty'}">${i <= Math.floor(rating) ? '⭐' : '☆'}</span>`;
    }
    return stars;
};
...
function renderRecipes(list) {
    ...
    if (list.length === 0) {
        container.innerHTML = `<div class="no-results"><p>No results found.</p><button onclick="renderRecipes(recipes)">Show all</button></div>`;
        return;
    }

    container.innerHTML = list.map(recipeTemplate).join('');
}
